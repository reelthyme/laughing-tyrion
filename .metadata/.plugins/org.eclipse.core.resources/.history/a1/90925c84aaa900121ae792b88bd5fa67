...
public class QuadTreeNode<T extends Collidable> {
	...
	final int MAX_PIECES_BEFORE_SPLIT_ATTEMPT = 1; //spltting is not implemented, elements
		//automatically go to the deepest level allowed by MAXDEPTH and their bounding box
	 ...
	 */
	 public int getIndex(Rectangle rect) {
	   ...
	   boolean topQuadrant = (rect.getMinY()>(cy-height/2) && rect.getMaxY() < cy);
	 ...
	 class MyFill implements ShapeFill{ //determines the color that the quad is rendered in debug mode
			@Override
			public Color colorAt(Shape shape, float x, float y) {
				...
				else if(DEPTH==4){return Color.blue;} //blue
